-- Rank OR matches by tag overlap. Same RPC, still one round trip, no new tables.
...
comment on function public.search_therapists is
  'OR tag overlap on specialty/insurance; rank by match_count then name; return matched_labels; optional format + license state; min rate + duration; cap 24.';
