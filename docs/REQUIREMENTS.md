...
10. **Admin test profile** — An admin stays `admin` and can open Join to publish one therapist profile on that same account (for testing signup). Submitting Join again replaces that profile, including a leftover therapist row. Find and `/t/[id]` show it when open to new clients. This is not a second login and not a multi-profile directory. A therapist or patient account still cannot join twice. The delete control stays on a therapist edit screen only.
